...
description: >-
  Inspect a reference website and output a reusable HaliteWebDevelopment **custom
  template** (manifest.json, sections/*.html, site.default.json, config.default.json,
  styles.css, scripts.js, assets) — NOT a Next.js app. Use when the user wants to
  extract a Halite template, create a template variant, bridge a cloned design to Halite
  Studio, or run /extract-halite-template. Requires referenceUrl, genre, and styleSlug.
argument-hint: "<referenceUrl> <genre> <variantSlug> [haliteRepoPath]"
user-invocable: true
---
...
## What This Produces (custom template — required)

Read `{haliteRepoPath}/studio/notes/halite-custom-template-contract.md` first.

```
templates/{genre}/{variantSlug}/
  manifest.json          # required — renderer: "custom"
  sections/*.html        # required — cloned DOM per section with data-halite-path
  site.default.json      # required — Latin placeholders only
  config.default.json    # required
  styles.css             # required
  scripts.js             # when reference has carousels/interactions
  assets/images/         # optional
```

**Do NOT** only copy `templates/{genre}/default/` JSON and tweak colors. That is forbidden.
...
2. **Custom HTML, not schema-only** — write real cloned section HTML in `sections/`. Fit data into `site.default.json`; do not invent Halite built-in DOM.
