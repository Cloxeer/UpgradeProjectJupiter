      ...
      <main>
        <section style={{ backgroundColor: "#003047" }}>
          <div className="pj-container py-14 text-center">
            ...
            <p className="mx-auto mt-5 max-w-[760px] text-gold" style={{ fontSize: 20, lineHeight: 1.4, fontWeight: 600 }}>
              Do not cancel it. Force the upgrade.
            </p>
          </div>
        </section>
        ...
        <section>
          <div className="pj-container py-14">
            <div className="mx-auto max-w-[820px]" style={{ fontSize: 17, lineHeight: 1.65, color: "#3c3c3c" }}>
              {petitionText.map((p, i) => (
                <p key={i} className={i === 0 ? "font-bold" : "mt-4"}>
                  {p}
                </p>
              ))}
              <ol className="mt-6 space-y-3 pl-6" style={{ listStyle: "decimal" }}>
                {petitionDemands.map((d) => (
                  <li key={d}>{d}</li>
                ))}
              </ol>
              <p className="mt-6">
                Same land. Same timeline. About 1.5% more money. Clean air, new water, local food, and
                about 3,000 permanent jobs instead of 750 that they signed for. Their own plan, finished properly.
              </p>
            </div>
            <div className="mt-12">
              <PetitionForm />
