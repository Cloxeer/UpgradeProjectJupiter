...
/** Our plan's gas hours reach zero in 2045 (HB93), so our uncaptured CO₂ stops growing after 17 operating years. */
export const OUR_GAS_OP_YEARS = 17;
export const ourGasOp = (y: number) => Math.min(operating(y), OUR_GAS_OP_YEARS);
...
export const rows: Row[] = [
  {
    ...
    theirs: (y) => (far(y) ? `${mt(GHG_PERMIT_TPY * GHG_PRACTICE_FACTOR * operating(y))} to ${mt(GHG_PERMIT_TPY * operating(y))} if run as filed for the whole period (estimate). Warming tracks the cumulative total and lasts centuries, so every ton is still in the air.` : operating(y) === 0 ? "0 so far (still building)" : `${mt(GHG_PERMIT_TPY * GHG_PRACTICE_FACTOR * operating(y))} to ${mt(GHG_PERMIT_TPY * operating(y))}`),
    ours: (y) => (far(y) ? `${mt(OUR_RELEASED_TPY_LOW * ourGasOp(y))} to ${mt(OUR_RELEASED_TPY_HIGH * ourGasOp(y))} in total (estimate): the uncaptured share through 2045, then zero gas hours. The captured share is rock inside concrete and aggregate, or CO₂ under cap rock long past its 50-year federal monitoring period.` : operating(y) === 0 ? "0 so far (still building)" : `${mt(OUR_RELEASED_TPY_LOW * ourGasOp(y))} to ${mt(OUR_RELEASED_TPY_HIGH * ourGasOp(y))}`),
    how: "Theirs: NMED's permitted 10,144,115 tons a year (low end uses the developers' own '40% less in practice' claim) × operating years, continued as filed for the whole period in the long views. Ours: the 5–10% not captured × operating years until 2045, when Process 4 reaches zero gas hours under HB93; nothing is added after that. Operations start in year 2, their own Q3 2028 target. Every figure past year 80 is an estimate that continues the documented trend.",
  ...
  },
  {
    ...
    theirs: (y) => (far(y) ? bgal(CONSTRUCTION_PUMPED_GAL + POTABLE_CAP_GPD * 365 * y) + " taken at the signed cap alone (estimate), plus undisclosed non-potable use every year. The 2000–2020 decline of the fresh table, continued for 250 years, leaves the wells under the towns dry or brackish; no filed plan or monitoring duty reaches this far." : y === 0 ? bgal(CONSTRUCTION_PUMPED_GAL) + " already pumped for construction" : bgal(CONSTRUCTION_PUMPED_GAL + POTABLE_CAP_GPD * 365 * y) + " (plus undisclosed non-potable use)"),
    ours: (y) => (far(y) ? bgal(OUR_WATER_GPD * 365 * operating(y)) + " of clean water made and " + bgal(recharged(y)) + " put back into the fresh aquifer (estimate, trends continued). The table under the towns is back near its 1980 level by about 2150 and holds there; the brine sits below the confining layers; the monitoring wells, paid for by the closure bond, are the record." : operating(y) === 0 ? "Same construction water, then the plant opens" : "Same offices, same cap, but " + bgal(OUR_WATER_GPD * 365 * operating(y)) + " of new clean water put into those pipes" + (y >= RECHARGE_START_YEAR ? ", and " + bgal(recharged(y)) + " put back into the fresh aquifer (low estimate)" : "")),
  ...
  },
  {
    ...
    theirs: (y) => (far(y) ? `Up to ${(250 * operating(y)).toLocaleString()} tons of each smog pollutant over the period if run as filed (estimate), over a valley that already fails the ozone standard, with only annual self-reports to show for it.` : operating(y) === 0 ? "Construction dust and truck exhaust in an area that already fails the ozone standard" : "Up to ~250 tons/yr of each smog pollutant added to an area that has failed the ozone standard since 2018 and gets an F grade"),
    ours: (y) => (far(y) ? "Zero stack emissions after 2045 (estimate: Process 4 as planned). Before that, every ton measured continuously and published, and the archive of every stack, hour by hour, is the record." : "Same fuel cells, but every ton measured continuously and published, under one permit with PSD-level controls"),
