...
export const glossary: Record<GlossaryKey, { term: string; short: string; long: string; kid: string; sources?: string[] }> = {
  ...
  PSD: { term: "PSD", short: "Prevention of Significant Deterioration: the strict review for big polluters", long: "A federal permitting review that applies when a plant emits 250 tons a year or more of a pollutant. It requires the best available controls. The fuel-cell permit's pollutants are far below that line (NOx 37 tons a year), so the review does not apply; the ask on this site is continuous monitoring instead.", kid: "the strict pollution check for big plants", sources: ["sob"] },
