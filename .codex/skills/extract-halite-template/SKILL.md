...
description: >-
  Inspect a reference website and output a reusable HaliteWebDevelopment JSON
  template (site.default.json, config.default.json, styles.css, assets) — NOT a
  Next.js app. Use when the user wants to extract a Halite template, create a
  template variant, bridge a cloned design to Halite Studio, or run
  /extract-halite-template. Requires referenceUrl, genre, and variantSlug.
